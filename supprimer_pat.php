...
if(isset($_GET['deleteid'])){
    ...
    $delete_std=mysqli_query($conn,"DELETE FROM `patient` where `user_id`=$id ");
...
<nav class="navbar navbar-expand-lg navbar-info bg-info" style="background-color: #0e5add !important;">
    ...
    <h1>Gérer les données des Patients:</h1>
    <table>
        <tr>
            <td>
                Matricule:
        ...
        <?php 
         if($result){//if it's true
            while($row=mysqli_fetch_assoc($result)){//recupère résultats de la ressource de résultats sous forme de tableau associatif
                ...
                $num_doss_med=$row['num_dossier_medicale'];
                ...
                <a href="modifier_pat.php? updateid= '.$matricule.'">Modifier</a></button>
                
                <button style="background-color:#33a0ff;
                color: #ffffff;
                cursor: pointer;"><a href="supprimer_pat.php? deleteid= '.$user_id.'">Supprimer</a></button>
                </td>
                </tr>';
